<?php



/**
 * This class defines the structure of the 'portfolio_company' table.
 *
 *
 *
 * This map class is used by Propel to do runtime db structure discovery.
 * For example, the createSelectSql() method checks the type of a given column used in an
 * ORDER BY clause to know whether it needs to apply SQL to make the ORDER BY case-insensitive
 * (i.e. if it's a text column type).
 *
 * @package    propel.generator.portfolio.map
 */
class PortfolioCompanyTableMap extends TableMap
{

    /**
     * The (dot-path) name of this class
     */
    const CLASS_NAME = 'portfolio.map.PortfolioCompanyTableMap';

    /**
     * Initialize the table attributes, columns and validators
     * Relations are not initialized by this method since they are lazy loaded
     *
     * @return void
     * @throws PropelException
     */
    public function initialize()
    {
        // attributes
        $this->setName('portfolio_company');
        $this->setPhpName('PortfolioCompany');
        $this->setClassname('PortfolioCompany');
        $this->setPackage('portfolio');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('id', 'Id', 'INTEGER', true, null, null);
        $this->addColumn('start', 'Start', 'INTEGER', true, null, null);
        $this->addColumn('end', 'End', 'INTEGER', true, null, null);
        $this->addColumn('company', 'Company', 'VARCHAR', true, 255, null);
        $this->addColumn('job', 'Job', 'VARCHAR', true, 255, null);
        $this->addColumn('description', 'Description', 'VARCHAR', true, 255, null);
        $this->addColumn('type', 'Type', 'VARCHAR', true, 45, null);
        $this->addColumn('city', 'City', 'VARCHAR', true, 45, null);
        $this->addColumn('zipCode', 'Zipcode', 'INTEGER', true, null, null);
        // validators
    } // initialize()

    /**
     * Build the RelationMap objects for this table relationships
     */
    public function buildRelations()
    {
    } // buildRelations()

} // PortfolioCompanyTableMap
